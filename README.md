<div align="center">

# ⚡ Jaykumar Tapodhan | Full-Stack Portfolio

**Full-Stack Developer · AI Enthusiast · Computer Engineering Undergraduate**

</div>

<div align="justify">

Building intelligent web apps, enterprise infrastructure, and AI systems at the intersection of full-stack engineering and product ownership.

> **“From enterprise LAN monitoring managing 2,000+ endpoints to AI-powered platforms — I ship things that work.”**

I am a Computer Engineering student (graduating 2027) at MBIT, CVM University, with production experience across AI integration, backend engineering, and enterprise systems. I have a track record of owning projects end-to-end under real-world deadlines, most recently as a Full-Stack Developer Intern at **Hi Lab Solution**, an AI Technology Intern at **Flaunch**, and a Full-Stack Development Trainee at **IBM**.

</div>

---

<h2 align="center">🛠️ Technical Arsenal</h2>

<div align="center">

  <b>Languages</b><br>
  <img src="https://img.shields.io/badge/python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54" alt="Python" />
  <img src="https://img.shields.io/badge/typescript-%23007ACC.svg?style=for-the-badge&logo=typescript&logoColor=white" alt="TypeScript" />
  <img src="https://img.shields.io/badge/javascript-%23323330.svg?style=for-the-badge&logo=javascript&logoColor=%23F7DF1E" alt="JavaScript" />
  <img src="https://img.shields.io/badge/c++-%2300599C.svg?style=for-the-badge&logo=c%2B%2B&logoColor=white" alt="C++" />
  <img src="https://img.shields.io/badge/c-%2300599C.svg?style=for-the-badge&logo=c&logoColor=white" alt="C" />
  <img src="https://img.shields.io/badge/sql-%234479A1.svg?style=for-the-badge&logo=postgresql&logoColor=white" alt="SQL" />

  <br><br>

  <b>Frameworks & Infrastructure</b><br>
  <img src="https://img.shields.io/badge/react-%2320232a.svg?style=for-the-badge&logo=react&logoColor=%2361DAFB" alt="React" />
  <img src="https://img.shields.io/badge/Next-black?style=for-the-badge&logo=next.js&logoColor=white" alt="Next JS" />
  <img src="https://img.shields.io/badge/tailwindcss-%2338B2AC.svg?style=for-the-badge&logo=tailwind-css&logoColor=white" alt="TailwindCSS" />
  <img src="https://img.shields.io/badge/Framer_Motion-black?style=for-the-badge&logo=framer&logoColor=blue" alt="Framer Motion" />
  <img src="https://img.shields.io/badge/node.js-6DA55F?style=for-the-badge&logo=node.js&logoColor=white" alt="NodeJS" />
  <img src="https://img.shields.io/badge/express.js-%23404d59.svg?style=for-the-badge&logo=express&logoColor=%2361DAFB" alt="Express.js" />
  <img src="https://img.shields.io/badge/FastAPI-005571?style=for-the-badge&logo=fastapi" alt="FastAPI" />
  <img src="https://img.shields.io/badge/MongoDB-%234ea94b.svg?style=for-the-badge&logo=mongodb&logoColor=white" alt="MongoDB" />
  <img src="https://img.shields.io/badge/postgresql-4169e1?style=for-the-badge&logo=postgresql&logoColor=white" alt="PostgreSQL" />

  <br><br>

  <b>AI / ML & DevOps</b><br>
  <img src="https://img.shields.io/badge/LLaMA-000000?style=for-the-badge" alt="LLaMA" />
  <img src="https://img.shields.io/badge/Groq_API-FF6C37?style=for-the-badge" alt="Groq API" />
  <img src="https://img.shields.io/badge/docker-%230db7ed.svg?style=for-the-badge&logo=docker&logoColor=white" alt="Docker" />
  <img src="https://img.shields.io/badge/kubernetes-%23326ce5.svg?style=for-the-badge&logo=kubernetes&logoColor=white" alt="Kubernetes" />

</div>

---

<div align="justify">

<h2 align="center">🚀 Featured Projects</h2>

### 1. InfraEye — Enterprise LAN Monitoring & Asset Intelligence
Took end-to-end ownership to build an enterprise LAN monitoring platform currently managing 2,000+ endpoints. Features a master-agent architecture (Watchdog-Worker pattern on Windows), live CPU/RAM/disk charts via Recharts, and automated Excel reporting that cut audit prep from 3 hours to 2 minutes.
- **Stack:** FastAPI, React 18, MongoDB, Python, PyInstaller, Framer Motion

### 2. TaskFlow — Team Task Management System
A Kanban-based team productivity platform with drag-and-drop task management, a 3-tier role system, real-time notifications, and full audit trails. Validated with 40+ tasks at 100% reliability and sub-200ms API response times.
- **Stack:** React.js, Node.js, Express.js, MongoDB, JWT Auth, Tailwind CSS
- **Links:** [Live Demo](https://taskflow-ctm.vercel.app/) | [GitHub Repo](https://github.com/Ethical-21/taskflow-app)

### 3. Flavour with Fusion — AI Meal Planner
An AI-powered recipe platform using a 3-tier resolution engine (Spoonacular → Custom DB → Groq/LLaMA) to guarantee zero empty results. Features dietary customization, AI ingredient substitution, and a drag-and-drop weekly meal planner.
- **Stack:** Next.js, TypeScript, Firebase Auth, Groq API, LLaMA
- **Links:** [Live Demo](https://flavour-with-fusion-eight.vercel.app/) | [GitHub Repo](https://github.com/Ethical-21/flavour-with-fusion)

---

<h2 align="center">💼 Experience Highlights</h2>

- **Hi Lab Solution** (Full-Stack Developer Intern) — Engineered and shipped `InfraEye`, a massively scalable enterprise LAN monitor with robust crash recovery and OTA update delivery.
- **IBM** (Full-Stack Development Trainee) — Built `TaskFlow` following IBM SDLC practices. Selected for an exclusive recognition visit to the IBM GIFT City office for top cohort performance.
- **Flaunch** (AI Technology Intern) — Built `Flavour with Fusion` and engineered fallback-resilient LLM pipelines. Ranked Top 20 of all interns and earned direct entry to the Unleash LLM Hackathon finals.

---

<h2 align="center">🏆 Honors & Awards</h2>

- 🥈 **2nd Runner-Up (Rs. 10,000)** — Dev Summit 2026 National Hackathon (Jagannath University)
- 🌟 **Top 20 Intern** — Flaunch (Earned Rs. 5,000 stipend & Direct Hackathon Finals entry)
- 🏢 **IBM GIFT City Recognition** — Top performer in IBM cohort
- 📜 **IBM Certifications** — React, Git, Docker, Kubernetes, Microservices
- ☁️ **Google Arcade Program** — Completed certification program and earned Google merchandise

---

<h2 align="center">🤝 Let's Connect</h2>

Currently expanding into DevOps and seeking internship or full-time roles where engineering depth and product ownership drive meaningful impact.

- 💼 **LinkedIn:** [linkedin.com/in/jaytapodhan](https://www.linkedin.com/in/jaytapodhan/)
- 📧 **Email:** [jaytapodhan21@gmail.com](mailto:jaytapodhan21@gmail.com)
</div>

<br />

<div align="center">
  <b>Built with ❤️ by Jaykumar Tapodhan</b>
</div>
